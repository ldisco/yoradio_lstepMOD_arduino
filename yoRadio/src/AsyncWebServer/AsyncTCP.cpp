...
static int8_t _tcp_recv(void * arg, struct tcp_pcb * pcb, struct pbuf *pb, int8_t err) {
    ...
    if(pb){
        ...
        e->fin.pcb = pcb;
        e->fin.err = err;
        //close the PCB in LwIP thread (guard: arg may be NULL if tcp_arg was reset)
        if(e->arg != NULL){
            AsyncClient::_s_lwip_fin(e->arg, e->fin.pcb, e->fin.err);
        } else {
            if(pcb){ if(tcp_close(pcb) != ERR_OK) tcp_abort(pcb); }
        }
    }
    if (!_send_async_event(&e)) {
        free((void*)(e));
...
int8_t AsyncClient::_recv(tcp_pcb* pcb, pbuf* pb, int8_t err) {
    while(pb != NULL) {
        ...
        if(_pb_cb){
            ...
            // [FIX] Защита от assert в pbuf_free при ref==0 (double-free после rx timeout)
            if(b->ref > 0) {
                pbuf_free(b);
            }
        }
...
int8_t AsyncClient::_s_lwip_fin(void * arg, struct tcp_pcb * pcb, int8_t err) {
    if(arg == NULL){
        // tcp_arg was already set to NULL; just close/abort the PCB to avoid leak
        if(pcb){ if(tcp_close(pcb) != ERR_OK) tcp_abort(pcb); }
        return ERR_OK;
    }
...
int8_t AsyncServer::_accept(tcp_pcb* pcb, int8_t err){
    //ets_printf("+A: 0x%08x\n", pcb);
    // [FIX] Защита от LoadProhibited: pcb или err могут быть невалидны при race condition в lwIP
...
int8_t AsyncServer::_s_accept(void * arg, tcp_pcb * pcb, int8_t err){
    // [FIX] Защита от LoadProhibited: arg == NULL возможен после tcp_arg(_pcb, NULL) в end(),
    // но lwIP успел принять новое соединение до очистки колбэка. Без этой проверки
    // reinterpret_cast<AsyncServer*>(NULL)->_accept(...) вызывает panic LoadProhibited.
    if(arg == NULL){
        if(pcb){
            if(tcp_close(pcb) != ERR_OK) tcp_abort(pcb);
        }
