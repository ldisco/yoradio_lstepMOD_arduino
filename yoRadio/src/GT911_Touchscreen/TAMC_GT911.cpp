...
void TAMC_GT911::begin(uint8_t _addr) {
  ...
  // [FIX] Задержка перед первым обращением к GT911 — чип может быть не готов
  //        после подачи питания. Устраняет I2C NACK ошибку при первой перезагрузке.
  delay(100);
  reset();
  // После сброса проверяем доступность (с повторными попытками)
  isOnline = false;
  for (int attempt = 0; attempt < 3 && !isOnline; attempt++) {
...
void TAMC_GT911::read(void) {
  // Если тач-контроллер недоступен, не делаем постоянные I2C-транзакции.
  // Это устраняет «бесконечный дождь» ошибок i2cRead/i2cWrite в мониторе.
  if (!isOnline) {
    touches = 0;
    isTouched = false;
    if (millis() < _nextProbeMs) {
      return;
    }
    Wire.beginTransmission(addr);
...
void TAMC_GT911::readBlockData(uint8_t *buf, uint16_t reg, uint8_t size) {
  ...
  uint8_t got = Wire.requestFrom(addr, size);
  if (got < size) {
    isOnline = false;
    _nextProbeMs = millis() + 5000;
