...
void Audio::showID3Tag(const char* tag, const char* value) {
    ...
    if(!strcmp(tag, "TP1")) { id3tag.appendf("Lead artist(s)/Lead performer(s)/Soloist(s)/Performing group: %s", value, "id3tag"); if(audio_id3artist) audio_id3artist(value); }
    ...
    if(!strcmp(tag, "TT2")) { id3tag.appendf("Title/Songname/Content description: %s", value, "id3tag"); if(audio_id3title) audio_id3title(value); }
    ...
    if(!strcmp(tag, "TIT2")) { id3tag.appendf("Title: %s", value, "id3tag"); if(audio_id3title) audio_id3title(value); }
...
//            info(evt_image, vec);
            // Прямой вызов колбэка audio_id3image для FLAC: передаём файл, позицию и размер встроенной картинки.
    ...
    if(m_controlCounter == FLAC_VORBIS) { /* VORBIS COMMENT */ // field names
        ...
        for(int i = 0; i < commentListLength; i++) {
            ...
            if(startsWith(commentString.get(), "artist") || startsWith(commentString.get(), "ARTIST")){
                ...
                audio_id3artist(artist); }  // [FIX] Был закомментирован — артист не вызывался для OGG/FLAC

            if(startsWith(commentString.get(), "title") || startsWith(commentString.get(), "TITLE")){
                title = strdup(commentString.get() + 6);
                audio_id3title(title); }  // [FIX] Был audio_id3album — title маппился на album для OGG/FLAC
...
//                info(evt_image, vec);
                // Прямой вызов колбэка audio_id3image для MP3: передаём файл, позицию и размер APIC-блока.
                if(audio_id3image) audio_id3image(m_audiofile, m_ID3Hdr.APIC_pos[0], m_ID3Hdr.APIC_size[0]);
                vec.clear();
...
//            info(evt_image, vec);
            // Прямой вызов колбэка audio_id3image для M4A: передаём файл, позицию и размер встроенной картинки.
            if(audio_id3image) audio_id3image(m_audiofile, m_m4aHdr.picPos, m_m4aHdr.picLen);
            vec.clear();
            audioFileSeek(pos); // the filepointer could have been changed by the user, set it back
        }
...
void Audio::processWebStream() {
    ...
    if(!m_f_running) return; // [FIX] После stopSong() не трогаем m_client — иначе LoadProhibited при смене/закрытии потока
    uint16_t readedBytes = 0;
...
void Audio::processWebFile() {
    ...
    if(!m_f_running) return; // [FIX] После stopSong() не трогаем m_client — иначе LoadProhibited
...
//                info(evt_name, icyName.get());
            ...
            // Передаём icy-description наверх как fallback‑тег для станций без метаданных.
...
void Audio::setDecoderItems() {
    ...
    //    setBitrate(MP3GetBitrate());
        uint32_t br = MP3GetBitrate();
        if(br) m_nominal_bitrate = br;  // Always use bitrate from file header if available
        if(m_nominal_bitrate) { AUDIO_INFO("BitRate: %lu", m_nominal_bitrate); }
    }
...
//                    info(evt_bitrate, "%i", m_cat.avrBitrateStable);
    ...
    if(m_haveNewFilePos && (m_cat.avrBitRate || m_cat.nominalBitRate)){
        ...
        m_haveNewFilePos = 0;
        m_cat.syltIdx = 0;
        // [FIX] Добавлена проверка что оба вектора не пусты
        if(m_syltLines.size() && m_syltTimeStamp.size()){
            while(m_cat.syltIdx < m_syltLines.size() && m_cat.syltIdx < m_syltTimeStamp.size()){
    ...
    if(m_syltLines.size()){
        ...
        // [FIX] Добавлена проверка границ m_syltTimeStamp для предотвращения crash
        if(m_cat.syltIdx >= m_syltTimeStamp.size()) return;
...
/*    m_gain0 = gainLowPass;
    ...
    int db = max(m_gain0, max(m_gain1, m_gain2)) + 10;
...
void Audio::IIR_calculateCoefficients(int8_t G0, int8_t G1, int8_t G2) { // Infinite Impulse Response (IIR) filters
    ...
    const float FcLS = 70;    // Frequency LowShelf[Hz] (500)
    const float FcPKEQ = 3000; // Frequency PeakEQ[Hz] (3000)
    float       FcHS = 9000;   // Frequency HighShelf[Hz] (6000)
...
int32_t Audio::getChunkSize(uint16_t *readedBytes, bool first) {
    ...
    if(!m_f_running) return -1; // [FIX] Не трогаем m_client после stopSong() — иначе LoadProhibited
...
bool Audio::readID3V1Tag() {
    ...
    if (InBuff.bufferFilled() == 128 && startsWith((const char*)p, "TAG")) {
        ...
        if (zeroByte == 0) {
            snprintf(chBuff.get(), 256, "Track Number: %d", track);
            /*info(evt_id3data,*/ audio_id3data(chBuff.get());
        }

        if (genre8 < 192) {
            snprintf(chBuff.get(), 256, "Genre: %d", genre8);
            /*info(evt_id3data,*/ audio_id3data(chBuff.get());
        }
