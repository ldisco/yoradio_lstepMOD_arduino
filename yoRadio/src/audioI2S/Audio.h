...
protected:
    // size_t            m_buffSize         = UINT16_MAX * 10;   // 655350 байт (~640КБ) - оригинал
    size_t            m_buffSize         = 774144;            // 756КБ буфер (v0.8.25)
...
//	    bool setPinout(uint8_t BCLK, uint8_t LRC, uint8_t DOUT, int8_t DIN = I2S_PIN_NO_CHANGE, int8_t MCK = I2S_PIN_NO_CHANGE);
    ...
    void loop();
    uint32_t stopSong();
    /** Текущий URL потока (для проверки при смене станции: мета/ошибка применяются только если совпадает с config.station.url). */
