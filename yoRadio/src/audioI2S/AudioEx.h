...
extern __attribute__((weak)) void audio_id3image(File& file, const size_t pos, const size_t size); //ID3 metadata image - раскомментировано для поддержки обложек из тегов SD
...
extern __attribute__((weak)) void audio_id3title(const char*);  // [FIX] Раскомментировано для поддержки SD тегов
...
/** Вызывается при получении данных из веб-потока (для watchdog: проверка «поток жив»). */
extern __attribute__((weak)) void audio_stream_activity(void);
//----------------------------------------------------------------------------------------------------------------------
...
//    bool audioFileSeek(const float speed);
    ...
    void         loop();
    uint32_t     stopSong();
    /** Текущий URL потока (для проверки при смене станции: мета/ошибка только если совпадает с config.station.url). */
    const char*  getLastHost() const { return m_lastHost.valid() ? m_lastHost.c_get() : ""; }
