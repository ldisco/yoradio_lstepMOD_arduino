...
static inline void appendGroup(char *buf, size_t bufSize, const char *name) {
  size_t used = strnlen(buf, bufSize);
  snprintf(buf + used, bufSize - used, "\"%s\",", name);
}
...
<hr />
<span>-= STATION COVERS =-<br />Select <i>.jpg</i> files for your stations</span>
<p><label for="covers">covers:</label><input type="file" name="covers" id="covers" accept=".jpg" multiple></p>
<hr />
...
(function(){
  // Ищем элемент футера с версией.
  var el = document.getElementById("version");
  if(!el) return;

  // Разбираем строку yoVersion на префикс и номер версии.
  var yv = String(yoVersion);
  var prefix = "";
  var ver = yv;
  // Регулярное выражение аналогично WebUI: всё до пробела + "vX.Y.Z" в конце.
  var m = yv.match(/^(.*\s)(v[\d.]+)$/);
  if(m){
    prefix = m[1]; // 
    ver = m[2];    // 
  }else{
  ...
  // Собираем HTML: разделитель, текстовый префикс и кликабельная версия.
  el.innerHTML = ' | ' + prefix +
...
<head>
  ...
  <script type="text/javascript" src="dragpl.js?v=2"></script>
  </head>
<body>
