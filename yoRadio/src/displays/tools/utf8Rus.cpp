...
char* utf8Rus(const char* str, bool uppercase) {
  ...
  for (int i = 0; str[i] && outPos < BUFLEN - 1; i++) {
    ...
    } else if (c == 0xE2 && (uint8_t)str[i + 1] == 0x80 && ((uint8_t)str[i + 2] == 0x93 || (uint8_t)str[i + 2] == 0x94)) {
      out[outPos++] = '-';
      i += 2;
    } else if ((c & 0xC0) == 0xC0) {      // Other multibyte (unknown)
      if ((c & 0xF0) == 0xE0) i += (str[i + 1] && str[i + 2] ? 2 : 0);
      else if ((c & 0xF8) == 0xF0) i += (str[i + 1] && str[i + 2] && str[i + 3] ? 3 : 0);
      else if ((c & 0xE0) == 0xC0) i += (str[i + 1] ? 1 : 0);
      out[outPos++] = ' '; // Skip unknown symbols or replace with space
    } else {                              // ASCII
    #if defined(DSP_LCD) && !defined(LCD_RUS)
